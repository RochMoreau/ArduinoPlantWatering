...
void draw_plant_data(int pumpId)
{
  ...
  else if (pumpIsActive[pumpId])
  {
    ...
    { // Pump is on, ready to work (not blocked by any delay) but not yet needed
...
void read_moisture_level(int pumpId)
{
  ...
  if (systemIsActive && pumpIsActive[pumpId] && !pumpIsBlocked[pumpId] &&
          (currentMillis - previousMillisMoisture[pumpId] > intervalMoisture[pumpId]) ||
      previousMillisMoisture[pumpId] == 0)
  {
...
void manage_pump(int pumpId)
{
  ...
  if (systemIsActive && pumpIsActive[pumpId] && !pumpIsRunning[pumpId] && !pumpIsBlocked[pumpId] && moistureLevel[pumpId] < moistureThreshold[pumpId] && moistureLevel[pumpId] >= lowestAcceptableLevel && moistureLevel[pumpId] <= highestAcceptableLevel && ((currentMillis - previousMillisPump[pumpId] > intervalPump[pumpId]) || previousMillisPump[pumpId] == 0))
  {
